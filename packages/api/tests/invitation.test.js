...
describe("Invitations API", () => {
  ...
  let adminToken;
    ...
    unauthorizedUserToken = responseUnauthorizedUser.body.token
		...
		const response = await request(app).get("/api/invitations/all").set('Authorization',  `Bearer ${adminToken}`).expect(200);
  
		publicInvitationId = response.body[0]._id;
    privateInvitationId = response.body[1]._id;
	}, 10000);
	afterAll(async () => {
		await request(app).delete("/api/invitations").set('Authorization',  `Bearer ${adminToken}`).expect(200);
    await request(app).delete("/api/auth").set('Authorization',  `Bearer ${adminToken}`).expect(200);
	});
    ...
    it("GET /api/invitations/all --> should throw an error is user is not admin", () => {
      return request(app).get("/api/invitations/all").set('Authorization',  `Bearer ${unauthorizedUserToken}`).expect("Content-Type", /json/).expect(401)
    })
	});
	...
	describe("deleteInvitations", () => {
        ...
        return request(app).delete(`/api/invitations`).set('Authorization',  `Bearer ${adminToken}`).expect(200);
    });
		it("DELETE /api/invitations --> should throw an error if user is not authorized to delete invitations", () => {
      return request(app).delete("/api/invitations").set('Authorization',  `Bearer ${unauthorizedUserToken}`).expect("Content-Type", /json/).expect(401)
    });
	});
});
