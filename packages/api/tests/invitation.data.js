const invitationAddTestData = {
  message: "Hiii! Wanna go to Edmonton with me on May 2?",
  messageImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExZW91d2Nyd2tjcHBpYndkNDFkaGNuNGhkYnUwMm1zN2M5dzdzbm43NSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/sRIY7gVfIV5AMwNbha/giphy.gif",
  responseEffect: true,
  response: "",
  successImage:
    "https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExNmZuZHF2YWtnbmd3bGRvZ3ZkbXBjOGo5ZjRqZHprNHFzZ2EydHUxbSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/3q2PV2UDy0VYKUvZSk/giphy.gif",
  successMessage: "Yayyyy!!! I'm excited!! :P",
  failedImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExeTF6Y2ZrYzRzOTE1NTgxNnNud2Q4d3NlYXFiZW8ydjQ3eDVsYnNsMiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/6qqfYC0LyAvWYKd8iW/giphy.gif",
  failedMessage: "awww :'(",
  recepient: "NY",
  isPublic: true,
  backgroundColor: "#f3ccd9",
  fontColor: "#fff9fb",
  buttonBackgroundColor: "#b25674",
  buttonFontColor: "#fff9fb",
};

const publicInvitationAddTestData = {
  message: "Hiii! Wanna go to Edmonton with me on May 2?",
  messageImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExZW91d2Nyd2tjcHBpYndkNDFkaGNuNGhkYnUwMm1zN2M5dzdzbm43NSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/sRIY7gVfIV5AMwNbha/giphy.gif",
  responseEffect: true,
  response: "",
  successImage:
    "https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExNmZuZHF2YWtnbmd3bGRvZ3ZkbXBjOGo5ZjRqZHprNHFzZ2EydHUxbSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/3q2PV2UDy0VYKUvZSk/giphy.gif",
  successMessage: "Yayyyy!!! I'm excited!! :P",
  failedImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExeTF6Y2ZrYzRzOTE1NTgxNnNud2Q4d3NlYXFiZW8ydjQ3eDVsYnNsMiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/6qqfYC0LyAvWYKd8iW/giphy.gif",
  failedMessage: "awww :'(",
  recepient: "NY",
  isPublic: true,
  backgroundColor: "#f3ccd9",
  fontColor: "#fff9fb",
  buttonBackgroundColor: "#b25674",
  buttonFontColor: "#fff9fb",
};

const privateInvitationAddTestData = {
  message: "Hiii! Wanna go to Calgary with me on May 21?",
  messageImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExZW91d2Nyd2tjcHBpYndkNDFkaGNuNGhkYnUwMm1zN2M5dzdzbm43NSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/sRIY7gVfIV5AMwNbha/giphy.gif",
  responseEffect: true,
  response: "",
  successImage:
    "https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExNmZuZHF2YWtnbmd3bGRvZ3ZkbXBjOGo5ZjRqZHprNHFzZ2EydHUxbSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/3q2PV2UDy0VYKUvZSk/giphy.gif",
  successMessage: "Yayyyy!!! I'm excited!! :P",
  failedImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExeTF6Y2ZrYzRzOTE1NTgxNnNud2Q4d3NlYXFiZW8ydjQ3eDVsYnNsMiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/6qqfYC0LyAvWYKd8iW/giphy.gif",
  failedMessage: "awww :'(",
  recepient: "NY",
  isPublic: false,
  backgroundColor: "#f3ccd9",
  fontColor: "#fff9fb",
  buttonBackgroundColor: "#b25674",
  buttonFontColor: "#fff9fb",
};

const invitationEditTestData = {
  message: "Hiii! Wanna go to Edmonton with me on May 2?",
  messageImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExZW91d2Nyd2tjcHBpYndkNDFkaGNuNGhkYnUwMm1zN2M5dzdzbm43NSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/sRIY7gVfIV5AMwNbha/giphy.gif",
  responseEffect: true,
  response: "yes",
  successImage:
    "https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExNmZuZHF2YWtnbmd3bGRvZ3ZkbXBjOGo5ZjRqZHprNHFzZ2EydHUxbSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/3q2PV2UDy0VYKUvZSk/giphy.gif",
  successMessage: "Yayyyy!!! I'm excited!! :P",
  failedImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExeTF6Y2ZrYzRzOTE1NTgxNnNud2Q4d3NlYXFiZW8ydjQ3eDVsYnNsMiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/6qqfYC0LyAvWYKd8iW/giphy.gif",
  failedMessage: "awww :'(",
  recepient: "NY",
  isPublic: true,
  backgroundColor: "#f3ccd9",
  fontColor: "#fff9fb",
  buttonBackgroundColor: "#b25674",
  buttonFontColor: "#fff9fb",
};

const invitationMissingMessageEditTestData = {
  messageImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExZW91d2Nyd2tjcHBpYndkNDFkaGNuNGhkYnUwMm1zN2M5dzdzbm43NSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/sRIY7gVfIV5AMwNbha/giphy.gif",
  responseEffect: true,
  response: "yes",
  successImage:
    "https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExNmZuZHF2YWtnbmd3bGRvZ3ZkbXBjOGo5ZjRqZHprNHFzZ2EydHUxbSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/3q2PV2UDy0VYKUvZSk/giphy.gif",
  successMessage: "Yayyyy!!! I'm excited!! :P",
  failedImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExeTF6Y2ZrYzRzOTE1NTgxNnNud2Q4d3NlYXFiZW8ydjQ3eDVsYnNsMiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/6qqfYC0LyAvWYKd8iW/giphy.gif",
  failedMessage: "awww :'(",
  recepient: "NY",
  isPublic: true,
  backgroundColor: "#f3ccd9",
  fontColor: "#fff9fb",
  buttonBackgroundColor: "#b25674",
  buttonFontColor: "#fff9fb",
};

const invitationMissingImageEditTestData = {
  message: "Hiii! Wanna go to Edmonton with me on May 2?",
  responseEffect: true,
  response: "yes",
  successImage:
    "https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExNmZuZHF2YWtnbmd3bGRvZ3ZkbXBjOGo5ZjRqZHprNHFzZ2EydHUxbSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/3q2PV2UDy0VYKUvZSk/giphy.gif",
  successMessage: "Yayyyy!!! I'm excited!! :P",
  failedImage:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExeTF6Y2ZrYzRzOTE1NTgxNnNud2Q4d3NlYXFiZW8ydjQ3eDVsYnNsMiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/6qqfYC0LyAvWYKd8iW/giphy.gif",
  failedMessage: "awww :'(",
  recepient: "NY",
  isPublic: true,
  backgroundColor: "#f3ccd9",
  fontColor: "#fff9fb",
  buttonBackgroundColor: "#b25674",
  buttonFontColor: "#fff9fb",
};

const invitationRegisterTestData = {
  email: "test3@test.com",
  password: "test123",
  name: "Test",
};

const invitationLoginTestData = {
  email: "test3@test.com",
  password: "test123",
};

const unauthorizedUserRegisterTestData = {
  email: "test4@test.com",
  password: "test123",
  name: "Test",
};

const unauthorizedUserLoginTestData = {
  email: "test4@test.com",
  password: "test123",
};

module.exports = {
  invitationAddTestData,
  publicInvitationAddTestData,
  privateInvitationAddTestData,
  invitationEditTestData,
  invitationMissingMessageEditTestData,
  invitationMissingImageEditTestData,
  invitationRegisterTestData,
  invitationLoginTestData,
  unauthorizedUserRegisterTestData,
  unauthorizedUserLoginTestData
};
